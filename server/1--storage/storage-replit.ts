...
export class ReplitStorage implements IStorage {
  ...
  async createUser(data: {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "⚠️ Failed to initialize preferences (non-fatal):",
          prefsErr,
        );
      }
      ...
      throw new Error(
        `User creation failed: ${err instanceof Error ? err.message : "Unknown error"}`,
      );
    }
  }
  ...
  async updateCompanyList(
    ...
    // Update index
    const companyIds =
      (await this.get<number[]>(`companies:list:${listId}`)) || [];
  ...
  async createCompany(company: InsertCompany): Promise<Company> {
    ...
    if (company.userId) {
      const userCompanies =
        (await this.get<number[]>(`companies:user:${company.userId}`)) || [];
    ...
    if (company.listId) {
      const listCompanies =
        (await this.get<number[]>(`companies:list:${company.listId}`)) || [];
    ...
    if (updates.listId !== undefined && updates.listId !== company.listId) {
      ...
      if (updates.listId) {
        const newListCompanies =
          (await this.get<number[]>(`companies:list:${updates.listId}`)) || [];
  ...
  async listContactsByCompany(companyId: number): Promise<Contact[]> {
    const contactIds =
      (await this.get<number[]>(`contacts:company:${companyId}`)) || [];
  ...
  async createContact(contact: InsertContact): Promise<Contact> {
    ...
    const companyContacts =
      (await this.get<number[]>(`contacts:company:${contact.companyId}`)) || [];
    ...
    if (contact.userId) {
      const userContacts =
        (await this.get<number[]>(`contacts:user:${contact.userId}`)) || [];
    ...
    const defaults = [
      {
        ...
        prompt:
          "Find key decision makers at the company who would be responsible for purchasing services.",
        order: 1,
        active: true,
        config: {
      ...
      },
      {
        ...
        prompt:
          "Perform a detailed analysis of the company, including size, services, and competitive advantages.",
        order: 2,
        active: true,
        config: {
      ...
      },
      {
        ...
        prompt:
          "Identify the most likely email patterns used by the company and generate possible emails for contacts.",
        order: 3,
        active: true,
        config: {
  ...
  async listCampaigns(userId: number): Promise<Campaign[]> {
    const campaignIds =
      (await this.get<number[]>(`campaigns:user:${userId}`)) || [];
  ...
  async createCampaign(
    ...
    const userCampaigns =
      (await this.get<number[]>(`campaigns:user:${campaign.userId}`)) || [];
  ...
  async addListToCampaign(
    ...
    const campaignLists =
      (await this.get<number[]>(`campaign:${campaignList.campaignId}:lists`)) ||
      [];
  ...
  async removeListFromCampaign(
    ...
    const campaignLists =
      (await this.get<number[]>(`campaign:${campaignId}:lists`)) || [];
  ...
  async listEmailTemplates(userId: number): Promise<EmailTemplate[]> {
    const templateIds =
      (await this.get<number[]>(`emailTemplates:user:${userId}`)) || [];
  ...
  async createEmailTemplate(
    ...
    const userTemplates =
      (await this.get<number[]>(`emailTemplates:user:${template.userId}`)) ||
      [];
  ...
  async getStrategicProfiles(userId: number): Promise<StrategicProfile[]> {
    try {
      const profileIds = await this.get<number[]>(
        `strategicProfiles:user:${userId}`,
      );
  ...
  async createStrategicProfile(
    ...
    const userProfiles = await this.get<number[]>(
      `strategicProfiles:user:${data.userId}`,
    );
  ...
  async deleteStrategicProfile(id: number, userId: number): Promise<void> {
    ...
    const userProfiles =
      (await this.get<number[]>(`strategicProfiles:user:${userId}`)) || [];
  ...
  async addContactFeedback(
    feedback: InsertContactFeedback,
  ...
  async listSearchTestResults(userId: number): Promise<SearchTestResult[]> {
    const resultIds =
      (await this.get<number[]>(`searchTestResults:user:${userId}`)) || [];
  ...
  async createSearchTestResult(
    ...
    const userResults =
      (await this.get<number[]>(`searchTestResults:user:${result.userId}`)) ||
      [];
    userResults.push(id);
    ...
    const strategyResults =
      (await this.get<number[]>(`testResults:strategy:${result.strategyId}`)) ||
      [];
    strategyResults.push(id);
    await this.set(
      `testResults:strategy:${result.strategyId}`,
      strategyResults,
    );
  ...
  async migrateFromPostgres(data: {
    ...
    if (data.lists?.length) {
      ...
      for (const list of data.lists) {
        ...
        // Add to user's lists
        const userLists =
          (await this.get<number[]>(`lists:user:${list.userId}`)) || [];
    ...
    if (data.contacts?.length) {
      ...
      for (const contact of data.contacts) {
        ...
        if (!companyContacts.includes(contact.id)) {
          ...
          await this.set(
            `contacts:company:${contact.companyId}`,
            companyContacts,
          );
        }
    ...
    if (data.campaigns?.length) {
      ...
      for (const campaign of data.campaigns) {
        ...
        if (campaign.campaignId > maxCampaignId)
          maxCampaignId = campaign.campaignId;
      }
      ...
      for (const campaignList of data.campaignLists) {
        ...
        const campaignLists =
          (await this.get<number[]>(
            `campaign:${campaignList.campaignId}:lists`,
          )) || [];
    ...
    if (data.emailTemplates?.length) {
      ...
      for (const template of data.emailTemplates) {
        ...
        const userTemplates =
          (await this.get<number[]>(
            `emailTemplates:user:${template.userId}`,
          )) || [];
    ...
    if (data.searchTestResults?.length) {
      ...
      for (const result of data.searchTestResults) {
        ...
        const userResults =
          (await this.get<number[]>(
            `searchTestResults:user:${result.userId}`,
          )) || [];
    ...
    console.log(
      "Migration from PostgreSQL to Replit DB completed successfully!",
    );
  }
}
