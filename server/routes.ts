...
Respond in this exact JSON format:
{
  ...
  app.post("/api/guest-user", async (req, res) => {
    try {
      ...
      if (!createGuest) {
        return res.status(400).json({ error: "createGuest flag required" });
      }

      // Create guest user with 72-hour TTL
      const expiresAt = new Date();
      expiresAt.setHours(expiresAt.getHours() + 72);

      const guestUser = await storage.createUser({
        username: `guest_${Date.now()}`,
        password: 'guest_password', // Not used for guests
        email: `guest_${Date.now()}@temp.local`,
        isGuest: true,
        expiresAt: expiresAt
      });

      console.log('Created guest user with 72-hour TTL:', { 
        id: guestUser.id, 
  ...
  // Cleanup expired guest users endpoint (can be called by cron job)
  app.post("/api/cleanup-expired-guests", async (req, res) => {
    try {
      const cleanupResult = await storage.cleanupExpiredGuests();
      
      console.log('Guest cleanup completed:', cleanupResult);
      
      res.json({
        message: 'Cleanup completed successfully',
        deletedUsers: cleanupResult.deletedUsers,
        deletedCompanies: cleanupResult.deletedCompanies,
        deletedContacts: cleanupResult.deletedContacts
      });

    } catch (error) {
      console.error('Guest cleanup error:', error);
      res.status(500).json({
        error: 'Failed to cleanup expired guests',
        message: error instanceof Error ? error.message : String(error)
      });
    }
  });
