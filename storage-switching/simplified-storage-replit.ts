/**
 * Simplified Replit Database Storage Implementation
 * This implementation uses Replit's key-value database for all storage operations
 */
...
export class ReplitStorage implements IStorage {
  ...
  async updateUserPreferences(userId: number, data: Partial<InsertUserPreferences>): Promise<UserPreferences> {
    const existing = await this.get<UserPreferences>(`user_preferences:${userId}`);
  ...
  async createCampaign(data: InsertCampaign & { userId: number }): Promise<Campaign> {
  ...
  async addListToCampaign(data: InsertCampaignList): Promise<CampaignList> {
    ...
    await this.set(`campaign_list:${data.campaignId}:${data.listId}`, campaignList);
    return campaignList;
  }
  ...
  async createEmailTemplate(data: InsertEmailTemplate & { userId: number }): Promise<EmailTemplate> {
  ...
  async addContactFeedback(data: InsertContactFeedback): Promise<ContactFeedback> {
  ...
  async createSearchTestResult(data: InsertSearchTestResult): Promise<SearchTestResult> {
  ...
  async updateTestResultStatus(id: number, status: 'completed' | 'running' | 'failed', metadata?: Record<string, unknown>): Promise<SearchTestResult> {
    ...
    const updated = { ...result, status, metadata: metadata || result.metadata };
