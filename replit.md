...
- June 21, 2025. Clean guest-to-registered transition system: Implemented comprehensive clean slate approach with preserved search query, localStorage cleanup, and tooltip reset for new registered users. Created reusable NotificationDialog component with "Hatchling" welcome message featuring black/white minimalist design. Added guest-cleanup utilities to remove orphaned search results while preserving user query in search input. Reset tooltip flags so new users see onboarding tooltips again. Welcome dialog highlights "Wildling-Duck-Family" branding and email search upgrade with single action button
