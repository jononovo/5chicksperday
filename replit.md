...
- June 20, 2025. Unified credit middleware system implementation: Created centralized credit-middleware.ts with requireCredits() and deductCreditsOnSuccess() functions for consistent credit management across all search endpoints. Applied middleware to "Find Key Emails" bulk endpoint (170 credits) and all individual contact action buttons - Hunter.io, Apollo.io, AeroLeads, and Perplexity enrichment (20 credits each). Eliminates billing gaps between bulk and individual email discovery methods, ensuring users are charged consistently regardless of search approach. Middleware adds only 5-13ms overhead vs 7-45 second search operations. All credit-charging endpoints now use unified pattern with proper error handling and graceful fallbacks
- June 20, 2025. AeroLeads UI streamlining completed: Hidden AeroLeads icon from ContactActionColumn component (both desktop tooltip button and mobile dropdown menu item) while preserving all backend functionality, API endpoints, state management, and credit integration. Users can no longer see or access AeroLeads through the UI, but the complete codebase remains intact for potential future re-enablement. Actions column now displays 3 buttons: Eye (view), Rocket (Apollo), Sparkles (AI), Target (Hunter)
