...
- June 17, 2025. Guest user TTL system implementation: Added 72-hour Time-To-Live system for guest users to maintain database hygiene and privacy compliance. Extended users table with isGuest boolean and expiresAt timestamp fields. Created guest user API endpoint (/api/guest-user) that generates temporary users with automatic 72-hour expiration. Implemented comprehensive cleanup system (/api/cleanup-expired-guests) that cascades deletion of expired guests and all related data (companies, contacts, lists, search jobs). Updated GuestUserManager to use correct static methods and integrated TTL creation into user registration flow. System enables automatic removal of abandoned guest accounts while preserving data continuity for users who register within the TTL window
